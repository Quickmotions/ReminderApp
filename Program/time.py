...
def update_reminder_times(initial_time ,initial_date ,repeats):
    ...
    while int(initial_time[:1]) <= int(current_time[:1]) and int(initial_time[3:]) <= int(current_time[3:]):
        if repeats == 'hourly':
            increase_hour(initial_time ,initial_date)
    while int(initial_time[:1]) <= int(current_time[:1]) and int(initial_time[3:]) <= int(current_time[3:]): # do for dates
        if repeats == 'daily':
            increase_day(initial_time ,initial_date)

# 2021-07-25
# 22:12
def increase_hour(initial_time ,initial_date ):
...
def increase_day(initial_time ,initial_date ):
...
def increase_month(initial_time ,initial_date ):



def increase_year(initial_time ,initial_date ):